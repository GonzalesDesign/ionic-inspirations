...
export class ConceptDetailsPage implements OnInit {
	...
	ionViewWillEnter() {
		...
		console.log(
			"this._afAuth.auth.currentUser.displayName--------: ",
			this._afAuth.auth.currentUser.displayName
		);

		/*--=| GETTING THE FIRST NAME FOR DISPLAY |=--*/
				...
				props: [
					{
						...
						custom_prop2: this.concept.avatarDescription,
						custom_prop3: this.concept.avatarUrl
					}
				]
			}
		});
		modal.onDidDismiss().then(selectedAvatar => {
			//   const avatar = data['x'];
			...
			this.concept.avatarDescription = dataObj.avatarDescription; //new value
		});
